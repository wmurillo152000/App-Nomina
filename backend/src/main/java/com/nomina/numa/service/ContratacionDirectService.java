...
public class ContratacionDirectService {

    private final MongoTemplate mongoTemplate;

    public List<Document> findAllAsDocuments() {
        log.info("=== ContratacionDirectService.findAllAsDocuments ===");
        try {
            MongoDatabase db = mongoTemplate.getDb();
            log.info("Database: {}", db.getName());

            MongoCollection<Document> collection = db.getCollection("contratacion");
            log.info("Collection: {}", collection.getNamespace().getCollectionName());

            long count = collection.countDocuments();
            log.info("Total documentos en contratacion: {}", count);
    ...
    public long count() {
        try {
            ...
            MongoCollection<Document> collection = db.getCollection("contratacion");
            long count = collection.countDocuments();
            log.info("Count contratacion: {}", count);
            return count;
        } catch (Exception e) {
